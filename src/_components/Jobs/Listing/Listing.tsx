...
import { TextArea } from '@/_components/Jobs/TextArea/TextArea';
import React, { useCallback, useEffect } from 'react';
import { FormButton } from '@/_components/Button/FormButton/FormButton';
import { Preview } from '@/_components/Jobs/ListingPreview/Preview';
...
export const Listing = ({ listingFromDb }: { listingFromDb?: IListing }) => {
  ...
  const triggerFormErrors = useCallback(() => trigger(), [trigger])
  
  useEffect(() => {
    void triggerFormErrors()
    return () => {
      togglePreview();
    };
  }, [togglePreview, triggerFormErrors]);
    ...
    <React.Fragment>
      ...
      >
        {newListingsInputFields.map(([labelText, fieldName], index: number) => {
          ...
          return (
            <div key={index}>
              <Input hasError={Boolean(errors[fieldName])} register={register} name={fieldName} labelText={labelText}
        ...
        <TextArea hasError={Boolean(errors.shortDescription)} register={register} fieldName='shortDescription' labelText='Short Description' />
        <TextArea hasError={Boolean(errors.fullDescription)} register={register} fieldName='fullDescription' labelText='Full Description' />
